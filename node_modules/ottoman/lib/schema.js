...
function _decodeValue(context, type, data) {
  if (data instanceof Object && data.$ref) {
    ...
    if ((modelType.type && modelType.type === 'Mixed') ||
      ...
      modelType = context.typeByName(data.klasse);

      if (!modelType) {
        throw new Error('Invalid type in mixed reference (' + data.klasse + ')');
      }
    }

    return modelType.ref(data.$ref);
  } else if (data instanceof Object && data.klasse) {
    if (type === mixedCoreType) {
      type = context.typeByName(data.klasse);
      if (!type) {
